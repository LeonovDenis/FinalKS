...
public class MultipleAxesLineChart extends StackPane {
    ...
    public Node getNewLegend() {
        HBox hBox = new HBox();


        Label label = new Label("Время от запуска, ч:мм:cc");
        //    final CheckBox baseChartCheckBox = new CheckBox(baseChart.getYAxis().getLabel());
        //     baseChartCheckBox.setSelected(true);
        //       baseChartCheckBox.setStyle("-fx-text-fill: " + toRGBCode(chartColorMap.get(baseChart)) + "; -fx-font-weight: bold;");
        //       baseChartCheckBox.setDisable(true);
...
/**
        final CheckBox baseCheckBox = new CheckBox("Инфо");
        baseCheckBox.setSelected(false);
        baseCheckBox.setStyle("-fx-text-fill: " + "green" + "; -fx-font-weight: bold;");


        baseCheckBox.selectedProperty().addListener(new ChangeListener<Boolean>() {
            @Override
        ...
        hBox.getChildren().add(baseCheckBox);*/
 ...
 hBox.getChildren().add(checkBox);
    ...
    private class DetailsPopup extends VBox {
        ...
        private HBox buildPopupRow(MouseEvent event, Long xValueLong, LineChart lineChart) {
            ...
            Number yValueLower = Math.round(normalizeYValue(lineChart, event.getY() - 1/*0*/));
            Number yValueUpper = Math.round(normalizeYValue(lineChart, event.getY() + 1/*0*/));
    ...
    public ObservableList<LineChart> getBackgroundCharts() {
        return backgroundCharts;
    }

    public LineChart getBaseChart() {
