...
public class ChinaDevice implements DetectorDevice, DetectorDevice.ChinaSource {
    ...
    /**
     * На подачу напряжения на клапан
     */
    private class SetMPowerTask extends DetectorTask {

        private final AtomicReference<FT_STATUS> result = new AtomicReference<FT_STATUS>();

        private  volatile AtomicReference<Byte> value = new AtomicReference<>();


        public SetMPowerTask(DetectorDevice device) {
            super(device);
        }
        ...
        public FT_STATUS setMPower(byte value) {
            ...
            try {
                ...
                LOG.error("Error while setting SetMPowerTask", e);
            }
            return result.get();
        }
        ...
        protected void handle() {
            ...
            result.set(grabber.setMPower(value.get()));
    ...
    public FT_STATUS setMPower(byte value) {
        FT_STATUS result = null;
        result = new SetMPowerTask(this).setMPower(value);
        if (result != FT_STATUS.FT_OK) {
            LOG.error("Error while setMPower: {}", result);
        }
        return result;
    }
