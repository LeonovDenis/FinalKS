...
public class ComList {
    ...
    private static volatile IntegerProperty tempProp =new SimpleIntegerProperty(0);
    ...
    {
        tempProp.addListener((observable, oldValue, newValue) -> {
            int i = (int) ((5000 / Math.pow(2, 16)) * newValue.intValue());
            StendParams.setTempValue(i);
        });
    }
    ...
    byte[] MPOWER= new byte[]{(byte) 0x05, (byte) 0x0C};

    /**
     * Регулировка напряжения на выходах по клапан
     * @param value
     * 0x01 – подача 12 В
     * 0x03 – подача 24 В
     * 0x00 – снятие напряжения
     * @return
     */
    public FT_STATUS setMPower(byte value) {

        Bytes msg = HEADER              //маска+ID
                .append(MPOWER[0])      //функция
                .append((byte) 0x02)    //размер[команда+данные]||
                .append(MPOWER[1])      //команда               |
        ...
        if (bytesData.startsWith(HEADER.array())) {
            ...
            switch (function) {
                ...
                case 0x28://Пишем первую часть кадра
                    ...
                    do {
                        try {
                            TimeUnit.MILLISECONDS.sleep(7);//должно быть 5
                    ...
                    if (bytes.length() < (2 * grabber.getHeight() * grabber.getWidth())) {
                        ...
                        return null;
                    } else if (bytes.length() == (2 * grabber.getHeight() * grabber.getWidth())+2) {

                        Bytes temp = bytes.copy(bytes.length() - 2, 2);
                        String hexTemp = temp.encodeHex(true);
                        int tempValue = temp.resize(4).toInt();
                        tempProp.set(tempValue);
                        int i = (int) ((5000 / Math.pow(2, 16)) * tempValue);
                     //   System.out.println(hexTemp+": "+tempValue+": "+i);
                        bytes= bytes.resize(bytes.length()-2, BytesTransformer.ResizeTransformer.Mode.RESIZE_KEEP_FROM_ZERO_INDEX);
                    }
    ...
    private Bytes testData(Bytes bytes) {
        if (count == 0) {
            ...
            //16384 значения
            Bytes length = Bytes.from((int) 16385).reverse().resize(3, BytesTransformer.ResizeTransformer.Mode.RESIZE_KEEP_FROM_ZERO_INDEX);
            ...
            bytes = bytes.append(Bytes.parseHex("0x400D"));
        } else {
            count = -1;
