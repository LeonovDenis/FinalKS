...
public class Utils {
     ...
     * @return
     */
    public static BufferedImage fillTempImage(BufferedImage src, ArrayList<BadPoint> bpList, int DIAMETR) {

        final int KVADRAT = 32 + 1;
        ...
        g2Loc.drawOval((int) Math.round((iw - DIAMETR) / 2.0), (int) Math.round((ih - DIAMETR) / 2.0),
                (DIAMETR - 1), (DIAMETR - 1));
     ...
     */
    public static void drawRect(Graphics2D g2, int wNature, int hNature,
                                int rectW, int rectH, int diametr,
        ...
        drawOval(g2, wNature, hNature, diametr, x1, y1, sizeKoef);
    }
     ...
     * @param sizeKoef коэфициент пикселя.
     */
    public static void drawOval(Graphics2D g2, int wNature, int hNature,
                                int diam,
                                int x1, int y1, double sizeKoef) {

        int rx = (int) (x1 + ((wNature - diam) / 2.0) / sizeKoef);
        int ry = (int) (y1 + ((hNature - diam) / 2.0) / sizeKoef);

        g2.setColor(new Color(0, 0, 0, 255));

        double[] anchors = calculate2anchors(wNature, hNature, diam);

        for (int i = 0; i < 2; i++) {

            g2.drawArc(rx, ry, (int) ((diam) / sizeKoef), (int) ((diam) / sizeKoef),
                    180 * i + (int) Math.round(anchors[1]),
                    (int) Math.round(anchors[0] - anchors[1]));

            g2.drawArc(rx, ry, (int) ((diam) / sizeKoef), (int) ((diam) / sizeKoef),
                    180 * i + 180 - (int) Math.round(anchors[0]),
                    (int) Math.round(anchors[0] - anchors[1]));
    ...
    /**
     * Полкучение углов пересечения квадрата с окружностью
     *
     * @param w    ширина квадрата
     * @param h    высота квадрата
     * @param diam диаметр круга
     * @return массив из двух углов
     */
    private static double[] calculate2anchors(int w, int h, int diam) {
        double[] doubles = new double[2];
        if (diam <= h || diam <= w) {
            return new double[]{90, 0};
        }
        double v = (0.5 * h) / (diam * 0.5);
        doubles[0] = Math.toDegrees(Math.asin(v));
        v = (0.5 * w) / (diam * 0.5);
        doubles[1] = Math.toDegrees(Math.acos(v));
        return doubles;
    }
    ...
    public static double[] makeMaxMeanMinNew(double[][] dataArrays, boolean noCorrection, double FALSE, int diametr) {
    ...
    /**
     * Определение точки внутри диаметра
     *
     * @param x       координата
     * @param y       координата
     * @param diametr диаметр круга
     * @return true -внутри, false -снаружи
     */
    public static boolean isPointInDiametr(int x, int y, int diametr, double w, double h) {

        double xCenter = (w - 1) / 2.0;
        double yCenter = (h - 1) / 2.0;

        double length = Math.round(Math.sqrt(Math.pow(x - xCenter, 2) + Math.pow(y - yCenter, 2)));
        return (length <= diametr / 2.0);
    }
    ...
    public static int parseIntText(ActionEvent event, boolean select, int maxValue) {
        TextField source = (TextField) event.getSource();
        String text = source.getText().trim();
        if (select) {
            source.selectAll();
     ...
     * @return
     */
    public static int bpInDiametr(ArrayList<Utils.Frame> frList, int position, int diametr) {
        Frame frame = frList.get(position);
        ArrayList<BadPoint> bpList = frame.getBpList();
        int w = frame.getSizeX();
        int h = frame.getSizeY();
        ...
        for (BadPoint bp :
            ...
            if (isPointInDiametr(bp.getX(), bp.getY(), diametr, w, h)) {
                count++;
            }
        }
    ...
    /**
     * Подсчет дефектности в диаметре
     *
     * @param frList  перечень
     * @param diametr диаметр
     * @param sizeX   размер матрицы по ширине
     * @param sizeY   размер матрицы по высоте
     * @return
     */
    public static int bbpInDiametr(List<BadBigPoint> frList, int diametr, int sizeX, int sizeY) {
    ...
    double ic = 63.5;
    double jc = 63.5;
    double d = Math.round(Math.sqrt(Math.pow((ic - i), 2) + Math.pow((jc - j), 2)));
}
