...
public class DetectorPanel extends JPanel implements DetectorListener {
    ...
    public class DefaultPainter implements Painter {
        ...
        public void paintPanel(DetectorPanel owner, Graphics2D g2) {
            if (controller != null&&!diamProp.isBound()) {
                diamProp.bind(controller.getParams().diametrProperty());
            }
        ...
        public void paintImage(DetectorPanel owner, BufferedImage image, Graphics2D g2) {
            ...
            if (isAimDisplayed() && s != 0) {
                drawRect(g2, iw, ih, aimWidth, aimHeight,diametr, x, y, s);
    ...
    private IntegerProperty diamProp = new SimpleIntegerProperty(0);
    private static int diametr = 0;
    /**
     * Пауза чтения кадров
     */
    ...
    public DetectorPanel(Detector detector, boolean start, boolean asinc) {
        this(detector, null, start, asinc,null);
    }
     ...
     * @param start    true, если детектор должен запускаться автоматически
     */
    public DetectorPanel(Detector detector, Dimension size, boolean start, boolean asinc, Controller controller) {
    ...
    public DetectorPanel(Detector detector, Dimension size, boolean start, ImageSupplier supplier,
        ...
        diamProp.addListener((observable, oldValue, newValue) -> {
            if (newValue.intValue() <= 0) {
                diametr = oldValue.intValue();
            } else {
                diametr = newValue.intValue();
            }
        });
    }
